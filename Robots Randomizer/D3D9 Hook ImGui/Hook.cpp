...
/**
	@brief : Function that unhook Reset and EndScene functions, and also disable the window message handler.
**/
...
{
	// Log that hkReset was called
	OutputDebugString("hkReset called.\n");

	// Set position flag and cleanup ImGui before reset
	Drawing::bSetPos = true;

	// Invalidate ImGui device objects before the reset
	if (Drawing::bInit)
	{
		OutputDebugString("Invalidating ImGui device objects before reset.\n");
		ImGui_ImplDX9_InvalidateDeviceObjects();
	}

	// Call the original Reset function and store the result
	OutputDebugString("Calling original Reset function.\n");
	HRESULT result = oReset(pPresentationParameters);

	if (result == D3D_OK)
	{
		OutputDebugString("Reset successful, reinitializing ImGui.\n");

		// If the reset was successful, reinitialize ImGui and rehook window procedure
		if (!Drawing::bInit && Hook::pDevice)
		{
			OutputDebugString("Calling Drawing::InitImGui.\n");
			Drawing::InitImGui(Hook::pDevice);

			// Recreate ImGui device objects after the reset
			OutputDebugString("Recreating ImGui device objects after reset.\n");
	...
	else
	{
		...
		char errorMsg[256];
		sprintf_s(errorMsg, "Device Reset Failed: 0x%08X\n", result);
